...
parser = argparse.ArgumentParser(
    description="cashloris, low bandwidth stress test tool for websites"
)
...
def main():
    ...
    logging.info("%s adlı ip %s socket ile uygulanıyor..", ip, socket_count)

    logging.info("Socket'ler oluşturuluyor.")
    ...
    while True:
        try:
            logging.info(
                "Gönderme işlemi uygulanıyor... Socket Sayısı: %s",
                len(list_of_sockets),
            )
            ...
            logging.info("Stopping cashloris")
            break
